...
import { Search, Phone, Building2, Eye, FileText, LayoutGrid } from 'lucide-react'
import Link from 'next/link'
import { toast } from 'sonner'
import { TowerView } from './tower-view'
import { AllocatedUnit } from './tower-actions'
...
interface LookupClientProps {
  towerAllocations: AllocatedUnit[]
}

export function LookupClient({ towerAllocations }: LookupClientProps) {
  const [activeTab, setActiveTab] = useState<'search' | 'tower'>('search')
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'search' && (
        <div className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-zinc-600">Search by serial number, purchaser name, or mobile number</p>
          </div>
            ...
            <div className="space-y-3 max-w-3xl mx-auto">
              {[1, 2, 3].map((i) => (
                <Card key={i} className="border-zinc-200 shadow-sm">
                  <CardContent className="py-4"><Skeleton className="h-20 w-full" /></CardContent>
                </Card>
              ))}
            </div>
          )}
            ...
            <Card className="border-zinc-200 shadow-sm p-12 text-center max-w-3xl mx-auto">
              <div className="flex flex-col items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-zinc-600 mt-1">Try searching with a different term</p>
                </div>
              </div>
            </Card>
          )}
          ...
          {!loading && results.length > 0 && (
            <div className="space-y-3 max-w-3xl mx-auto">
              <p className="text-sm text-zinc-600">Found {results.length} result{results.length !== 1 ? 's' : ''}</p>
              {results.map((result) => (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <Badge variant="outline" className="font-mono">{result.serial_display}</Badge>
                          <Badge
                        ...
                        <h3 className="text-lg font-semibold text-zinc-900 mb-2">{result.applicant_name}</h3>
                        <div className="space-y-1 text-sm text-zinc-600">
                          <div className="flex items-center gap-2">
                            <Phone className="w-4 h-4" />{result.applicant_mobile}
                          </div>
                          <div className="flex items-center gap-2">
                            <Building2 className="w-4 h-4" />{result.project_name} • Unit {result.unit_no}
                          </div>
                          <div className="flex items-center gap-2">
                            <FileText className="w-4 h-4" />Booking Amount: {formatCurrency(result.booking_amount_paid)}
                          </div>
                        </div>
                      </div>
            ...
            <Card className="border-zinc-200 shadow-sm p-12 text-center max-w-3xl mx-auto">
              <div className="flex flex-col items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-zinc-600 mt-1">Enter a serial number, name, or mobile to find bookings</p>
                </div>
              </div>
            </Card>
          )}
      ...
      {/* Tower tab */}
      {activeTab === 'tower' && (
        <TowerView initialAllocations={towerAllocations} />
      )}
    </div>
  )
}
